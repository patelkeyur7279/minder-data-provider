...
function checkReactVersions() {
  ...
  try {
    ...
    const isMonorepo = fs.existsSync(path.join(projectRoot, 'demo')) || 
    ...
    // Check main package
    try {
      const mainReact = path.join(projectRoot, 'node_modules', 'react', 'package.json');
      const mainReactDom = path.join(projectRoot, 'node_modules', 'react-dom', 'package.json');
      ...
      if (fs.existsSync(mainReactDom)) {
        const version = JSON.parse(fs.readFileSync(mainReactDom, 'utf8')).version;
    ...
    if (isMonorepo) {
      ...
      for (const workspace of workspaces) {
        ...
        const reactPath = path.join(workspacePath, 'node_modules', 'react', 'package.json');
        const reactDomPath = path.join(workspacePath, 'node_modules', 'react-dom', 'package.json');
        
        if (fs.existsSync(reactPath)) {
          const version = JSON.parse(fs.readFileSync(reactPath, 'utf8')).version;
          reactVersions.set(workspace, version);
          log(`  ✓ ${workspace} React: ${version}`, 'green');
        }
        
        if (fs.existsSync(reactDomPath)) {
          const version = JSON.parse(fs.readFileSync(reactDomPath, 'utf8')).version;
    ...
    if (issues.length > 0) {
      ...
      log('  3. Ensure React is only in peerDependencies, not dependencies/devDependencies', 'cyan');
