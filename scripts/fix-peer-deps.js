...
function fixVersionIssues() {
  ...
  try {
    ...
    const mainReactDomPath = path.join(
      projectRoot,
      "node_modules",
      "react-dom"
    );
    ...
    if (isLibrary) {
      ...
      if (packageJson.dependencies && packageJson.dependencies.react) {
        log(
          "  ⚠️  React found in dependencies (should be in peerDependencies only)",
          "yellow"
        );
      ...
      if (
        ...
        log(
          "  💡 For libraries, React should only be in peerDependencies",
          "cyan"
        );
    ...
    if (fs.existsSync(demoPath)) {
      ...
      if (fs.existsSync(demoPackageJsonPath)) {
        ...
        if (
          !demoPackageJson.dependencies ||
          !demoPackageJson.dependencies.react
        ) {
