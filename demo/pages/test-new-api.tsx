    ...
    <div style={{ padding: '2rem', fontFamily: 'system-ui', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <div
          style={{
            padding: '1rem',
            background: usersLoading ? '#fef3c7' : users ? '#d1fae5' : '#fee2e2',
            borderRadius: '8px',
            marginBottom: '1rem',
          }}
        >
        ...
        {users && (
          <pre style={{ background: '#f5f5f5', padding: '1rem', borderRadius: '8px', overflow: 'auto', maxHeight: '200px' }}>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        >
          {postsLoading ? '⏳ Loading...' : posts ? `🔄 Refresh ${posts.length} posts` : '📥 Fetch Posts'}
        </button>
        {postsError && <p style={{ color: '#ef4444', marginTop: '1rem' }}>❌ {postsError.message}</p>}
        {posts && (
          <pre style={{ background: '#f5f5f5', padding: '1rem', borderRadius: '8px', overflow: 'auto', maxHeight: '200px', marginTop: '1rem' }}>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        >
          {todosLoading ? '⏳ Loading...' : todos ? `🔄 Refresh ${todos.length} todos` : '📥 Fetch Todos'}
        </button>
        {todosError && <p style={{ color: '#ef4444', marginTop: '1rem' }}>❌ {todosError.message}</p>}
        {todos && (
          <pre style={{ background: '#f5f5f5', padding: '1rem', borderRadius: '8px', overflow: 'auto', maxHeight: '200px', marginTop: '1rem' }}>
...
{`const result = await minder('/posts/1', 
  ...
  { method: 'PUT' }
      ...
      <div style={{ background: '#dbeafe', border: '2px solid #3b82f6', padding: '1.5rem', borderRadius: '8px' }}>
        <h3 style={{ margin: '0 0 0.5rem 0', color: '#1e40af' }}>✅ Working!</h3>
        <p style={{ margin: 0, color: '#1e40af' }}>
          The universal <code style={{ background: 'white', padding: '0.25rem 0.5rem', borderRadius: '4px' }}>minder()</code> function is working perfectly with pure TypeScript functions!
        </p>
      </div>
    </div>
  );
