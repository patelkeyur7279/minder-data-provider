...
import { useWebSocket } from '../../src/hooks/index.js';
...
export function WebSocketRealTimeExample() {
  ...
  useEffect(() => {
    ...
    const unsubscribeNotification = ws.subscribe('notification', (data: any) => {
    ...
    const unsubscribePresence = ws.subscribe('user-presence', (data: any) => {
      ...
      if (data.action && data.user) {
        const systemMessage: Message = {
          id: Math.random().toString(36).substr(2, 9),
          type: 'system',
          content: `${data.user.name} ${data.action === 'join' ? 'joined' : 'left'} the chat`,
          sender: 'System',
          timestamp: Date.now()
        };
  ...
  const getMessageStyle = (message: Message) => {
    ...
    switch (message.type) {
      case 'chat':
        return `${baseStyle} ${message.metadata?.isOwn ? 'own-message' : 'other-message'}`;
      case 'system':
        return `${baseStyle} system-message`;
      case 'notification':
        return `${baseStyle} notification-message ${message.metadata?.level || 'info'}`;
      default:
        return baseStyle;
    }
  };
    ...
    <div className="websocket-realtime">
      ...
      <div className="connection-status-panel">
        ...
        <div className="status-grid">
          ...
          <div className="status-item">
            <strong>Last Ping:</strong> {stats.lastPing ? formatTimestamp(stats.lastPing) : 'None'}
          </div>
        </div>
      ...
      <div className="feature-explanation">
        ...
        <ul>
          <li><strong>🔌 useWebSocket():</strong> Hook for WebSocket connection management</li>
          <li><strong>📨 Message Subscription:</strong> Subscribe to specific message types with callbacks</li>
          <li><strong>🔄 Auto-Reconnection:</strong> Automatic reconnection with exponential backoff</li>
          <li><strong>🏓 Heartbeat/Ping:</strong> Connection health monitoring with ping/pong</li>
          <li><strong>👥 Presence System:</strong> Real-time user presence and status updates</li>
          <li><strong>💬 Real-Time Chat:</strong> Instant messaging with typing indicators</li>
          <li><strong>🔔 Notifications:</strong> System-wide notification broadcasting</li>
          <li><strong>⚡ User Actions:</strong> Real-time user activity tracking</li>
          <li><strong>📊 Connection Stats:</strong> Detailed connection and message statistics</li>
          <li><strong>🔒 Authentication:</strong> Token-based WebSocket authentication</li>
        </ul>
      </div>
