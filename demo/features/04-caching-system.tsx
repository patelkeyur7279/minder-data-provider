...
export function CachingSystemExample() {
  ...
  const handleCheckFreshness = () => {
    ...
    alert(`Cache Status:\nUsers: ${usersFresh ? 'Fresh' : 'Stale'}\nProducts: ${productsFresh ? 'Fresh' : 'Stale'}`);
  };
  ...
  const handleOptimisticUpdate = async () => {
    try {
      ...
      await cache.optimisticUpdate(
        'users',                          // Query key to update
        (oldData: any[]) => {            // Updater function
        ...
        () => {                          // Rollback function (called on error)
          ...
          alert('Optimistic update failed and was rolled back');
        }
      );
      ...
      // Simulate API failure after 3 seconds to show rollback
      setTimeout(() => {
        console.log('❌ Simulating API failure...');
        // This would normally be triggered by an actual API error
      }, 3000);
  ...
  return (
    <div className="caching-system">
      ...
      <div className="feature-explanation">
        ...
        <ul>
          <li><strong>📥 Prefetching:</strong> Load data into cache without UI updates</li>
          <li><strong>⏰ TTL (Time To Live):</strong> Automatic cache expiration with staleTime</li>
          <li><strong>🗑️ Garbage Collection:</strong> Automatic cleanup with gcTime</li>
          <li><strong>🔍 Freshness Check:</strong> Determine if cached data is still fresh</li>
          <li><strong>🔄 Invalidation:</strong> Force refresh of specific or all cached data</li>
          <li><strong>💾 Manual Management:</strong> Direct cache read/write operations</li>
          <li><strong>🎯 Optimistic Updates:</strong> Immediate updates with rollback capability</li>
          <li><strong>🔑 Flexible Keys:</strong> Support for simple strings or complex nested keys</li>
          <li><strong>📊 Cache Analytics:</strong> Real-time statistics and monitoring</li>
          <li><strong>🧹 Cache Cleanup:</strong> Granular or complete cache clearing</li>
        </ul>
      </div>
